...
The `home-routes` handler in the `htmx-cool.routes.home` namespace
...
The `home-page` function will in turn call the `htmx-cool.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `htmx-cool.handler` namespace:
...
Request middleware functions are located under the `htmx-cool.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `htmx-cool.dev-middleware` namespace found in
...
* Let `mount` know to start the XTDB node by `require`-ing `htmx-cool.db.core` in some other namespace.
